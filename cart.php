...
if ($_SERVER['REQUEST_METHOD']=='POST') {
  ...
  if (empty($_SESSION['cart'][$id])) {
    $q=mysqli_query($connect,"SELECT * FROM song WHERE Song_id = '$id'");
 ...
 <div class="row">
  ...
  <br>
 	<?php 
	  $id =$_GET['id'];
	 $cnn = mysqli_connect("localhost","root","","sdlc");
	 $q=mysqli_query($cnn,"SELECT * FROM song WHERE Song_id = {$id}");
		...
		<h4><?php echo $item['Decription'] ?></h4>
        <?php
		echo "<a href='delcart.php?productid=$item[Song_id]' style='text-decoration: none;'></a>";
		?>
         </a>
         </div>
...
<h3 style="text-align: center;">Payment</h3>
 	<form method="POST" action="thanhtoan.php" class="was-validated">
 		<div class="form-group">
  		...
  		<input type="text" class="form-control" id="usr" name="name" value="<?php echo $_SESSION['user'];  ?>" readonly>
		</div>
...
<div class="form-group">
  ...
  <input type="text" class="form-control" id="usr" value=" <?p=$tong ." $" ?>" readonly>
</div>
<?php
$id =$_GET['id'];
 $q=mysqli_query($connect,"SELECT * FROM song WHERE Song_id = '$id'");
...
<input type="submit" class="btn btn-success" value="Pay">
 ...
 <a href="product.php"><button>Backup product</button></a>
